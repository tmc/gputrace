...
func (t *Trace) ExtractShaderMetrics() (*ShaderMetricsReport, error) {
...
func (t *Trace) populateThreadMetrics(metricsMap map[string]*ShaderMetrics) error {
	commandBuffers, err := t.ParseCommandBuffers()
	if err != nil {
		return err
	}
	...
	for _, cb := range commandBuffers {
		...
		for i, encoder := range dcb.Encoders {
			...
			if metrics, exists := metricsMap[encoder.Label]; exists {
				// Take the first dispatch configuration (they're usually the same)
				if metrics.ThreadgroupsX == 0 {
					metrics.ThreadgroupsX = dispatch.ThreadsX
					metrics.ThreadgroupsY = dispatch.ThreadsY
					metrics.ThreadgroupsZ = dispatch.ThreadsZ
					metrics.ThreadsPerGroupX = dispatch.ThreadsPerGroupX
					metrics.ThreadsPerGroupY = dispatch.ThreadsPerGroupY
					metrics.ThreadsPerGroupZ = dispatch.ThreadsPerGroupZ
				}
			}
		}
	}
...
func (t *Trace) estimateTimingMetrics(metricsMap map[string]*ShaderMetrics) error {
