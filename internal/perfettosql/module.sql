...
SELECT
  ...
  extract_arg(arg_set_id, 'debug.pipeline_compiler_remark_argument_availability') AS pipeline_compiler_remark_argument_availability,
  cast(extract_arg(arg_set_id, 'debug.pipeline_compiler_remark_argument_count') AS INT) AS pipeline_compiler_remark_argument_count,
  cast(extract_arg(arg_set_id, 'debug.pipeline_compiler_remark_argument_malformed_count') AS INT) AS pipeline_compiler_remark_argument_malformed_count,
  extract_arg(arg_set_id, 'debug.pipeline_compiler_remark_argument_count_semantics') AS pipeline_compiler_remark_argument_count_semantics,
  extract_arg(arg_set_id, 'debug.pipeline_compiler_remark_argument_semantics') AS pipeline_compiler_remark_argument_semantics,
  ...
  cast(extract_arg(arg_set_id, 'debug.presentation_pipeline_tracks') AS INT) AS presentation_pipeline_tracks,
  cast(extract_arg(arg_set_id, 'debug.presentation_pipeline_events') AS INT) AS presentation_pipeline_events,
  cast(extract_arg(arg_set_id, 'debug.presentation_encoder_tracks') AS INT) AS presentation_encoder_tracks,
  cast(extract_arg(arg_set_id, 'debug.presentation_encoder_events') AS INT) AS presentation_encoder_events,
  cast(extract_arg(arg_set_id, 'debug.presentation_uncertain_tracks') AS INT) AS presentation_uncertain_tracks,
  cast(extract_arg(arg_set_id, 'debug.presentation_uncertain_events') AS INT) AS presentation_uncertain_events,
...
SELECT
  ...
  cast(extract_arg(arg_set_id, 'debug.thread_invariant_spilled') AS INT) AS thread_invariant_spilled,
  cast(extract_arg(arg_set_id, 'debug.threadgroup_memory') AS INT) AS threadgroup_memory,
  cast(extract_arg(arg_set_id, 'debug.instruction_count') AS INT) AS instruction_count,
  cast(extract_arg(arg_set_id, 'debug.alu_instruction_count') AS INT) AS alu_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.fp32_instruction_count') AS INT) AS fp32_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.fp16_instruction_count') AS INT) AS fp16_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.int32_instruction_count') AS INT) AS int32_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.int16_instruction_count') AS INT) AS int16_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.branch_instruction_count') AS INT) AS branch_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.device_load_instruction_count') AS INT) AS device_load_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.device_store_instruction_count') AS INT) AS device_store_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.device_atomic_instruction_count') AS INT) AS device_atomic_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.texture_reads_instruction_count') AS INT) AS texture_reads_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.texture_writes_instruction_count') AS INT) AS texture_writes_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.threadgroup_load_instruction_count') AS INT) AS threadgroup_load_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.threadgroup_store_instruction_count') AS INT) AS threadgroup_store_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.threadgroup_atomic_instruction_count') AS INT) AS threadgroup_atomic_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.wait_instruction_count') AS INT) AS wait_instruction_count,
  cast(extract_arg(arg_set_id, 'debug.constant_calculation_temporary_register_count') AS INT) AS constant_calculation_temporary_register_count,
  cast(extract_arg(arg_set_id, 'debug.constant_calculation_phase_present') AS INT) AS constant_calculation_phase_present,
  cast(extract_arg(arg_set_id, 'debug.compilation_time_ms') AS REAL) AS compilation_time_ms,
  cast(extract_arg(arg_set_id, 'debug.recorded_statistic_count') AS INT) AS recorded_statistic_count,
  extract_arg(arg_set_id, 'debug.recorded_statistics_json') AS recorded_statistics_json,
  extract_arg(arg_set_id, 'debug.recorded_statistics_semantics') AS recorded_statistics_semantics,
  extract_arg(arg_set_id, 'debug.metrics_source') AS compiler_metrics_source,
  extract_arg(arg_set_id, 'debug.source') AS source,
  extract_arg(arg_set_id, 'debug.semantics') AS semantics,
  extract_arg(arg_set_id, 'debug.clock_domain') AS clock_domain,
  extract_arg(arg_set_id, 'debug.timing_quality') AS timing_quality,
...
SELECT
  ...
  cast(extract_arg(arg_set_id, 'debug.argument_count') AS INT) AS argument_count,
  extract_arg(arg_set_id, 'debug.arguments_json') AS arguments_json,
  extract_arg(arg_set_id, 'debug.argument_semantics') AS argument_semantics,
  extract_arg(arg_set_id, 'debug.source') AS source,
  extract_arg(arg_set_id, 'debug.semantics') AS semantics,
  extract_arg(arg_set_id, 'debug.clock_domain') AS clock_domain,
  extract_arg(arg_set_id, 'debug.timing_quality') AS timing_quality,
...
-- gputrace_pipeline_compiler_remark_arg preserves the ordered scalar Args
-- entries carried by one compiler remark. Values remain strings; this view
-- does not assign pass-specific meaning or runtime weight.
CREATE PERFETTO VIEW gputrace_pipeline_compiler_remark_arg AS
SELECT
  r.id AS remark_event_id,
  r.pipeline_id,
  r.function_name,
  r.remark_index,
  cast(json_extract(a.value, '$.index') AS INT) AS argument_index,
  json_extract(a.value, '$.name') AS argument_name,
  json_extract(a.value, '$.raw') AS argument_raw,
  json_extract(a.value, '$.raw_value') AS argument_raw_value,
  json_extract(a.value, '$.value') AS argument_value,
  json_extract(a.value, '$.parse_status') AS parse_status,
  r.source,
  r.clock_domain,
  r.timing_quality
FROM gputrace_pipeline_compiler_remark AS r, json_each(r.arguments_json) AS a;
