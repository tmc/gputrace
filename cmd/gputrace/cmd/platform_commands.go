...
var xcodeProfileCommandSpecs = []platformCommandSpec{
	{name: "run", use: "run <trace_file>", short: "Run full automation (open, replay, export)", args: cobra.ExactArgs(1), flags: outputFlag},
	{name: "open", use: "open <trace_file>", short: "Open a trace file in Xcode", args: cobra.ExactArgs(1), flags: foregroundFlag},
	{name: "close", use: "close [trace_file]", short: "Close the trace window in Xcode", args: cobra.MaximumNArgs(1)},
	{name: "export", use: "export [output_path]", short: "Export the trace from Xcode", args: cobra.MaximumNArgs(1)},
	{name: "run-profile", use: "run-profile [trace_file]", aliases: []string{"run-replay"}, short: "Start profiling in Xcode", args: cobra.MaximumNArgs(1)},
	{name: "wait-profile", use: "wait-profile [trace_file]", aliases: []string{"wait-replay"}, short: "Wait for profiling to complete", args: cobra.MaximumNArgs(1)},
	{name: "check-status", use: "check-status [trace_file]", short: "Check profiling status", args: cobra.MaximumNArgs(1), flags: debugFlag},
	{name: "check-permissions", use: "check-permissions", short: "Check required permissions", args: cobra.NoArgs},
	{name: "select-tab", use: "select-tab <tab_name>", short: "Select a tab in the trace viewer", args: cobra.ExactArgs(1)},
	...
	{name: "xcode-export-counters", use: "xcode-export-counters [trace_file]", short: "Export GPU counters from Xcode's Performance view to CSV", args: cobra.MaximumNArgs(1), flags: forceFlag},
	{name: "xcode-export-memory", use: "xcode-export-memory [trace_file]", short: "Export memory report from Xcode's Performance view", args: cobra.MaximumNArgs(1), flags: forceFlag},
	{name: "vertex-output", use: "vertex-output <trace.gputrace>", short: "Extract vertex shader output from Xcode GPU debugger", args: cobra.ExactArgs(1), flags: vertexOutputFlags},
	{name: "list-windows", use: "list-windows [trace_file]", short: "List Xcode windows", args: cobra.MaximumNArgs(1), hidden: true},
	...
	{name: "list-menus", use: "list-menus [menu-name]", short: "List menu bar items and their contents", args: cobra.MaximumNArgs(1), hidden: true},
	{name: "click-menu", use: "click-menu <menu> <item>", short: "Click a menu item", args: cobra.ExactArgs(2), hidden: true},
	{name: "list-buttons", use: "list-buttons", short: "List buttons using XCUIAutomation and AX", args: cobra.NoArgs, hidden: true},
	...
	{name: "open-export", use: "open-export [output_path]", short: "Open the export dialog and set the output path", args: cobra.MaximumNArgs(1), hidden: true},
	...
	{name: "screenshot", use: "screenshot [trace_file]", short: "Capture a screenshot of the Xcode window", args: cobra.MaximumNArgs(1), hidden: true, flags: screenshotFlags},
	{name: "debug-tree", use: "debug-tree [trace_file]", short: "Print UI tree to find key elements", args: cobra.MaximumNArgs(1), hidden: true, flags: verboseFlag},
	{name: "ensure-checked", use: "ensure-checked <checkbox_title>", short: "Ensure a checkbox is checked", args: cobra.ExactArgs(1), hidden: true, flags: traceFlag},
...
Subcommands:
  ...
  memory    Extract memory usage info when visible`,
	...
	for _, name := range []string{"show", "status", "overview", "timeline", "shaders", "counters", "cost-graph", "heat-map", "encoders", "cost", "summary", "memory"} {
		...
		cmd.AddCommand(&cobra.Command{Use: name, Short: performanceCommandShort(name), Hidden: hidden, Args: cobra.NoArgs, RunE: platformXcodeProfileRun("performance/" + name)})
	}
	return cmd
}
...
func newNavigatorCommand() *cobra.Command {
	cmd := &cobra.Command{Use: "navigator", Short: "Navigate to different sections in the Debug navigator", Hidden: true}
...
func init() {
	...
	rootCmd.AddCommand(newPlatformCommand(platformCommandSpec{name: "xcode-bindings", use: "xcode-bindings", short: "Inspect private Xcode GTShaderProfiler bindings", args: cobra.NoArgs, flags: jsonFlag}))
	rootCmd.AddCommand(newPlatformCommand(platformCommandSpec{name: "xcode-parity", use: "xcode-parity <trace.gputrace>", short: "Audit Xcode metric parity for a trace", args: cobra.ExactArgs(1), flags: jsonFlag}))
}
