...
| `GPUTRACE_CAPTURE_FLUSH_MS` | CUPTI periodic flush interval in milliseconds (default 100; 0 disables it). The period bounds how much a target that exits without synchronizing loses: on MLX decode, 500ms lost between a quarter and a half of the records, 100ms and 25ms lost none. |
